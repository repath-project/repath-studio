...
(defn input-key-down-handler!
  "Generic on-key-down handler for uncontrolled input elements that dispatches an event
   `f` in order to update a db value on keyboard enter, or reset to the initial value `v`
   on escape.

   We need uncontrolled inputs to avoid updating the canvas with incomplete values while
   the user is typing, and also avoid polluting the history stack.
