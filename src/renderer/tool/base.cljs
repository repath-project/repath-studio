...
;; Tool multimethods.
(defmulti pointer-down (fn [db _e _t] (:tool db)))
(defmulti pointer-move (fn [db _e _t] (:tool db)))
(defmulti pointer-up (fn [db _e _t] (:tool db)))
(defmulti double-click (fn [db _e _t] (:tool db)))
(defmulti drag (fn [db _e _t] (:tool db)))
(defmulti drag-start (fn [db _e _t] (:tool db)))
(defmulti drag-end (fn [db _e _t] (:tool db)))
(defmulti key-up (fn [db _e _t] (:tool db)))
(defmulti key-down (fn [db _e _t] (:tool db)))
(defmulti activate :tool)
(defmulti deactivate :tool)
(defmulti properties keyword)

(defmethod pointer-down :default [db _e _t] db)
(defmethod pointer-up :default [db _e _t] db)
(defmethod pointer-move :default [db _e _t] db)
(defmethod drag-start :default [db _e _t] db)
(defmethod double-click :default [db _e _t] db)
(defmethod key-up :default [db _e _t] db)
(defmethod key-down :default [db _e _t] db)
(defmethod drag :default [db e t] (pointer-move db e t))
(defmethod drag-end :default [db e t] (pointer-up db e t))
